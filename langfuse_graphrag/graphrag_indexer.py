...
Return ONLY the JSON array, nothing else."""
    ...
    def _build_communities_hierarchical(
        ...
        for level in range(max_levels):
            ...
            for cluster_id, entity_indices in clusters.items():
                ...
                community = {
                    ...
                    "level": level,
                    # Don't set summary here - it will be generated by LLM
                    "_entity_indices": entity_indices,  # Temporary, for parent-child linking
                    "_entity_ids": [entity_ids[i] for i in entity_indices],  # Keep for summary generation
                }
        ...
        # Remove temporary fields (but keep _entity_ids for summary generation)
        for comm in communities:
            comm.pop("_entity_indices", None)
            # Keep _entity_ids for now - will be removed after summary generation
    ...
    def generate_summaries(
        ...
        # Create a mapping of entity ID to entity dict for quick lookup
        entity_map = {entity.get("id"): entity for entity in semantic_entities}
        
        # Create a mapping of community ID to entities that belong to it
        # Use the _entity_ids stored during community building
        community_entity_map = defaultdict(list)
        for community in communities:
            entity_ids = community.get("_entity_ids", [])
            for entity_id in entity_ids:
                if entity_id in entity_map:
                    community_entity_map[community.get("id")].append(entity_map[entity_id])
        ...
        for level in range(max_level, -1, -1):
            ...
            for community in level_communities:
                ...
                # Get entities that belong to this community
                community_entities = community_entity_map.get(community_id, [])
                
                # Get child community summaries if this is a parent community
                child_summaries = []
                if level < max_level:
                    # Find child communities (communities at higher levels that might be children)
                    # This is approximate - we'll use communities with similar names at higher levels
                    for child_level in range(level + 1, max_level + 1):
                        if child_level in communities_by_level:
                            for child_comm in communities_by_level[child_level]:
                                child_name = child_comm.get("name", "").lower()
                                comm_keyword = community_name.split(":")[1].strip().lower() if ":" in community_name else community_name.lower()
                                if comm_keyword in child_name and child_comm.get("summary"):
                                    child_summaries.append(child_comm.get("summary"))
                
                # Build entity information for the prompt
                entity_info = ""
                if community_entities:
                    # Use actual entity names and descriptions
                    entity_list = []
                    for entity in community_entities[:15]:  # Limit to 15 entities
                        name = entity.get("name", "")
                        desc = entity.get("description", "")
                        if desc:
                            entity_list.append(f"- {name}: {desc[:100]}")
                        else:
                            entity_list.append(f"- {name}")
                    entity_info = "\n".join(entity_list)
                else:
                    # Fallback: use relevant chunks
                    community_type = community_name.split(":")[0].lower() if ":" in community_name else ""
                    relevant_chunks = []
                    for chunk in chunks:
                        text = chunk.get("text", "").lower()
                        if community_type and community_type in text:
                            relevant_chunks.append(chunk.get("text", "")[:300])
                        if len(relevant_chunks) >= 5:
                            break
                    if relevant_chunks:
                        entity_info = "\n\n".join(relevant_chunks)
                
                # Build prompt with entity information
                if entity_info or child_summaries:
                    child_context = ""
                    if child_summaries:
                        child_context = f"\n\nSub-communities:\n" + "\n".join([f"- {s}" for s in child_summaries[:5]])
                    
                    prompt = f"""Generate a concise, informative summary (2-3 sentences) for this community of related concepts.

Community Name: {community_name}

Entities in this community:
{entity_info[:1500]}{child_context}

Provide a clear, descriptive summary that explains what this community represents, what themes or concepts it contains, and how these entities relate to each other. Be specific and avoid generic phrases like "related entities"."""
                    ...
                    try:
                        response = self._call_openai_with_retry(
                            lambda: self.openai_client.chat.completions.create(
                                model=self.llm_model,
                                messages=[
                                    {"role": "system", "content": "You are an expert at creating clear, informative summaries of related concepts. Focus on what the community represents, not just that it contains entities."},
                                    {"role": "user", "content": prompt}
                                ],
                                temperature=self.config.get("graphrag_llm_temperature", 0.0),
                                max_tokens=250,
                    ...
                    # No entities or chunks found, use descriptive default
                    comm_keyword = community_name.split(":")[1].strip() if ":" in community_name else community_name
                    community["summary"] = f"Community focused on {comm_keyword}"
        
        # Remove temporary _entity_ids field now that summaries are generated
        for comm in communities:
    ...
    def index(
        ...
        # Pass semantic entities so summaries can use actual entity information
        communities = self.generate_summaries(communities, chunks, semantic_entities)
